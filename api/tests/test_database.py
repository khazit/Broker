...
@pytest.fixture
def dummy_job_1():
    return Job(identifier=17, status=2) 
...
def test_init_db_warm(warm_db):
    assert warm_db.db_get_n_jobs() == 3
    assert warm_db.db_get_n_jobs(active=False) == 6 

def test_init_db_warm_empty(warm_empty_db):
    assert warm_empty_db.db_get_n_jobs() == 0
    assert warm_empty_db.db_get_n_jobs(active=False) == 0

def test_init_db_cold(cold_db):
    assert cold_db.db_get_n_jobs() == 0
    assert cold_db.db_get_n_jobs(active=False) == 0   

def test_add_job(warm_db, warm_empty_db, dummy_job_1, dummy_job_2):
    warm_db.db_add_job(dummy_job_1)
    assert warm_db.db_get_n_jobs() == 4
    assert warm_db.db_get_n_jobs(active=False) == 7

    warm_empty_db.db_add_job(dummy_job_2) 
    assert warm_empty_db.db_get_n_jobs() == 1
    assert warm_empty_db.db_get_n_jobs(active=False) == 1

def test_remove_job(warm_db, warm_empty_db):
    warm_db.db_remove_job(5)
    assert warm_db.db_get_n_jobs() == 3
    assert warm_db.db_get_n_jobs(active=False) == 5

    warm_db.db_remove_job(7)
    assert warm_db.db_get_n_jobs() == 2
    assert warm_db.db_get_n_jobs(active=False) == 4

    warm_empty_db.db_remove_job(17) 
    assert warm_empty_db.db_get_n_jobs() == 0
    assert warm_empty_db.db_get_n_jobs(active=False) == 0
